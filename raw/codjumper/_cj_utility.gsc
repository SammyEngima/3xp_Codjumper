...
setupAdvert()
{
	...
	if ( !isDefined( self.cj["hud"] ) || !isDefined( self.cj["hud"]["advert"] ) )
	{
		...
		self.cj["hud"]["advert"] setShader("hud_advert",234,30);
	}
...
changeAdvert()
{
	...
	if(isDefined(self.cj["hud"]["advert"]))
		self.cj["hud"]["advert"] setShader("hud_website",100,10);
}
...
createVoteHud()
{
	...
	if(!isDefined(self.cj["hud"]["vote"]))
	{
		...
		self.cj["hud"]["vote"].x = -92;
		self.cj["hud"]["vote"].y = -57;
	...
	if(!isDefined(self.cj["hud"]["votearg"]))
	{
		...
		self.cj["hud"]["votearg"].x = -92;
		self.cj["hud"]["votearg"].y = -44;
	...
	if(!isDefined(self.cj["hud"]["votetime"]))
	{
		...
		self.cj["hud"]["votetime"].x = -14;
		self.cj["hud"]["votetime"].y = -16;
	...
	if(!isDefined(self.cj["hud"]["votebg"]))
	{
		...
		self.cj["hud"]["votebg"].x = -5;
		self.cj["hud"]["votebg"].y = -5;
		self.cj["hud"]["votebg"].sort = -2;
		self.cj["hud"]["votebg"].alpha = 1;
		self.cj["hud"]["votebg"] setShader("hud_bg_vote", 170, 70 );
	}
	if(self codjumper\_cj_utility::get_cjcfg("CJ_HUD_WT")){
	  if(isDefined(self.cj["hud"]["vote"]))
